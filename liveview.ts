import {
  ...
  CameraProperty,
  watchCameras,
} from "./napi-canon-cameras/camera-api";
import { EventEmitter } from "events";

const events = new EventEmitter();
events.on(CameraBrowser.EventName.PropertyChangeValue, (event) => {
  if (event.property.identifier === CameraProperty.ID.Evf_OutputDevice) {
    console.log(event.property.label, event.property.value);
  }
});
events.on(CameraBrowser.EventName.LiveViewStart, (event) => {
  console.log("LV Started", event);
});
events.on(CameraBrowser.EventName.LiveViewStop, (event) => {
