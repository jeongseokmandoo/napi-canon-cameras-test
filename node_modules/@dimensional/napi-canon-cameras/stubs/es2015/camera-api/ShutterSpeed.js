...
export class ShutterSpeed {
    ...
    static getLabelForSeconds(seconds) {
        let label = '';
        if (seconds > 0.2999) {
    ...
    static findNearest(valueOrLabel, filter) {
        ...
        const found = Object.keys(ShutterSpeed.AllValues).reduce((carry, key) => {
    ...
    static forLabel(label) {
        ...
        const match = label.match(/(\d+(?:\.\d+)?)(?:\s*\/\s*(\d+))?(?:\s+(.*))?/);
        if (match) {
            const isOneThird = (match[3] || '').indexOf('1/3') >= 0;
            let seconds = parseFloat(match[1]) || 0.0;
            if (match[2]) {
                seconds /= parseFloat(match[2]);
            }
            const values = isOneThird ? ShutterSpeed.OneThirdValues : ShutterSpeed.OneHalfValues;
            const value = Object.keys(values).find((straw) => Math.abs(values[straw] - seconds) < 0.0000001);
